...
pub async fn fetch_prayer_times(lat: f64, lon: f64) -> Result<PrayerTimes, String> {
    ...
    let url = format!("http://api.aladhan.com/v1/timings?latitude={}&longitude={}&method=2", lat, lon);

    match client.get(&url).send().await {
        Ok(resp) => {
            match resp.json::<AladhanResponse>().await {
                Ok(data) => {
...
pub fn get_next_prayer(timings: &PrayerTimes) -> (String, String, String) {
    ...
    for (name, time_str) in &prayers {
        ...
        if let Ok(time) = NaiveTime::parse_from_str(time_str, "%H:%M") {
            let prayer_dt = now.date_naive().and_time(time).and_local_timezone(Local).unwrap();
            
            if prayer_dt > now {
                let diff = prayer_dt - now;
                next_prayer = Some((name.to_string(), time_str.to_string(), format_duration(diff)));
                break;
            }
        }
    }
    ...
    if let Some(res) = next_prayer {
        ...
        if let Ok(time) = NaiveTime::parse_from_str(time_str, "%H:%M") {
             ...
             let diff = prayer_dt - now;
             (name.to_string(), time_str.to_string(), format_duration(diff))
        } else {
